    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if hasattr(insect, 'container') and insect.container and insect.ant:
                    self.ant = insect.ant
                    insect.ant.place = self
                elif isinstance (insect, QueenAnt) and not insect.fake_queen:
                    self.ant = insect
                    insect.place = self
                else:
                    self.ant = None
...
class Ant(Insect):
    ...
    def can_contain(self, ant):
        if self.container:
            if self.ant == None and not ant.container:
                return True
        return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        # BEGIN Problem 3B
        "*** REPLACE THIS LINE ***"
        if self.place.bees != [] and self.min_range and not (10000 - self.max_range):
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6B
        self.bee = bee
        self.bee.reduce_armor(self.bee.armor)
        # END Problem 6B
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor =1):
        ...
        "*** REPLACE THIS LINE ***"
        self.armor = armor
        if not self.exsiting_queens:
            self.name = 'Queen'
            QueenAnt.exsiting_queens = True
        else:
    ...
    def action(self, colony):
        ...
        if self.fake_queen:
            ...
            while current_place.exit != None:
                self.place_behind.append(current_place.exit)
                current_place = current_place.exit
            for place in self.place_behind:
                if place.ant != None:
                    if not place.ant in self.inspiring_ants:
                        place.ant.damage = place.ant.damage * 2
                        self.inspiring_ants.append(place.ant)
                    if place.ant.container == True and place.ant.ant:
                        if not place.ant.ant in self.inspiring_ants:
                            place.ant.ant.damage = place.ant.ant.damage * 2
                            self.inspiring_ants.append(place.ant.ant)                
...
def apply_effect(effect, bee, duration):
    ...
    i = 0
    while i < duration:
        effect(bee.action)(bee)
        i += 1
    bee.be_in_effect = False
